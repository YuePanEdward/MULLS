## Available open datasets for LiDAR SLAM and point cloud registration test

### 64-line mechanical LiDAR

[KITTI Odometery Dataset](http://www.cvlibs.net/datasets/kitti/eval_odometry.php)

[KITTI Raw Data](http://www.cvlibs.net/datasets/kitti/raw_data.php)

[KITTI 360](http://www.cvlibs.net/datasets/kitti-360/user_login.php)

[Semantic KITTI](http://semantic-kitti.org/)

[Ford Campus Dataset](http://robots.engin.umich.edu/SoftwareData/Ford)

[Apollo SouthBay Localization Dataset](http://apolloscape.auto/self_localization.html)

[PandaSet](https://pandaset.org/#about-hesai)

[MulRan: Multimodal Range Dataset for Urban Place Recognition](https://sites.google.com/view/mulran-pr/home)

[New College Dataset](https://ori.ox.ac.uk/older-projects/new-college-dataset/)

### 32-line mechanical LiDAR

[UrbanLoco](https://advdataset2019.wixsite.com/urbanloco)

[LOAM NSH & Gates building Dataset](http://wiki.ros.org/loam_velodyne)

[Koide HDL SLAM Dataset](https://github.com/koide3/hdl_graph_slam)

[ISPRS Benchmark on multisensorial indoor mapping and positioning](http://mi3dmap.net/datatype3.jsp)

### 16-line mechanical LiDAR

[LEGO-LOAM Stevens Dataset](https://drive.google.com/drive/folders/1dIsonWdZNBGiUiGh2H6dWJKoJ1k8_A2j)

### single-line LiDAR

[ETH ASL Registration Dataset](https://projects.asl.ethz.ch/datasets/doku.php?id=laserregistration:laserregistration)

### Livox solid state LiDAR

[Livox HKU & HKUST Dataset](https://github.com/hku-mars/loam_livox)

[Livox Dataset](https://www.livoxtech.com/cn/dataset)

### Terristial laser scanning (TLS) registration

[ETH PRS Registration Dataset](https://prs.igp.ethz.ch/research/completed_projects/automatic_registration_of_point_clouds.html)

[WHU Regsitration Dataset](http://3s.whu.edu.cn/ybs/en/benchmark.htm)